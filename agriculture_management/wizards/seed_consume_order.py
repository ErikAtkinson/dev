...
class SeedConsumeOrder(models.TransientModel):
    """Seed Consume Order"""
    _name = 'seed.consume.order'
    ...
    def default_get(self, field):
        res = super(SeedConsumeOrder, self).default_get(field)
        active_ids = self._context.get('active_ids', [])
        filtered_seeds = self.env['farm.crop.seed'].browse(active_ids).filtered(lambda f: not f.delivery_order_id)
        res['farm_crop_seed_ids'] = [(6, 0, filtered_seeds.ids)]
        return res
    ...
    def action_create_consume_order(self):
        ...
        if all(record.delivery_order_id for record in self.farm_crop_seed_ids):
            message = {
            ...
            return message
        for line in self.farm_crop_seed_ids:
            lines.append((0, 0, {
                'product_id': line.seed_id.product_id.id,
                'product_uom_qty': line.qty,
                'product_uom': line.seed_id.product_id.uom_id.id,
                'location_id': self.warehouse_id.lot_stock_id.id,
                'location_dest_id': dest_location_id.id,
                'name': line.seed_id.product_id.name
            }))
